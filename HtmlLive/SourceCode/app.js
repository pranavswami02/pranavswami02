...
childRef.on('value', function(snap) {
    ...
    infoDiv.innerHTML = "<span class='key'>Name: </span><span class='value'>"+data[index].name+"</span> <span class='key'>Email: </span> <span class='value'>"+data[index].email+"</span> <span class='key'>ID: </span> <span class='value'>"+data[index].id+" <span class='key'>Short-Url: </span> <span class='value'><a href='"+data[index].url+"'>"+data[index].url+"</a></span><button class='copy' onclick='copyToClip()'>copy</button>"
